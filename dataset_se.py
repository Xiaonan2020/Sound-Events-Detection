...
class MelLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, metafile_path, cfg):
        ...
        self.sampling_rate = cfg['sampling_rate'] # 采样率
        self.device = cfg['device'] 
        # mel 计算类实例
        self.mel_calc_inst = MelSpec(cfg['win_len'],        # stft win: 512 傅里叶变换的窗长
                                     cfg['hop'],            # stride: 160
                                     cfg['nfilter'],        # mel calc win: 400 梅尔普计算的窗长
    ...
    def get_mel(self, wav_path):
        ...
        audio_norm = audio / self.max_wav_value # 赋值归一化 [-1, 1]
        audio_norm = audio_norm.unsqueeze(0) # [n,] --> [1, n]
        melspec = self.mel_calc_inst.mel_spectrogram(audio_norm)    # mel计算 # torch.Size([1, 40, 1001])
