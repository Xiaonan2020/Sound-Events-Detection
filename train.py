import os
from argparse import ArgumentParser  # 传一些 mate file 地址参数
...
def train():
    ...
    start_epoch = 0 # 初始化为0 用来恢复训练 传到def save_checkpoint的'epoch': epoch_
    iteration = 0 # 一共跑了多少步

    model = model.to(cfg['device'])
    
    if args.c:
        checkpoint = torch.load(args.c, map_location="cuda:0")
        model.load_state_dict(checkpoint['model_state_dict'])
        opt.load_state_dict(checkpoint['optimizer_state_dict'])
        start_epoch = checkpoint['epoch'] # 上次停止的epoch
        iteration = start_epoch*len(train_loader) # 开始的步数
        print('Resume training from %s' % args.c)
    else:
        print('trainig from scratch!')

    
    model.train() # drop=0, train_flag = True  model.eval()
