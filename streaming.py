...
FORMAT = pyaudio.paInt16  # 录制位宽：short  pyaudio调用麦克风
N_CHANNEL = 1   # 通道
SR = 16000  # 采样率
CHUCK_SIZE = int(10 * SR / 1000)  # 每次录制10ms 10ms一共录了多少点
chuck_pos = 0 # 当前录制的位置  麦克风录制到当前的点
ring_length = 100 * 60 * 60  # 环形buffer的总长：60 minutes  把麦克风录的数据放到环形buffer中，一直存，存到最后把前面的覆盖
ring_buffer = [np.array([0]*CHUCK_SIZE)]*ring_length # 环形buffer

# 特征抽取实例
# 从ring buffer中取10ms的数据，计算特征
feat = ap.MelSpec(
...
def recording(record_param):
    ...
    while 1:
        ...
        ring_buffer[chuck_pos] = data  # 不停的读麦克风，存到ring buffer中
        chuck_pos += 1
        if chuck_pos == ring_length: # reached the buffer tail 到头了就覆盖前面的
            chuck_pos = 0
...
def streaming(callback, n_sec=10):
    ...
    while 1:
        ...
        # 计算分类的结果
        ret = callback(data)  # callback-->predict
        prob = np.squeeze(ret)
